...
主数据是 UCI Online Retail 2010 年 12 月真实公开数据，42,481 行 × 8 列，CC BY 4.0。演示页是一次已完成、已独立复验的真实引擎运行快照，不是现场重跑，也不发起后端或模型请求。页面以面向使用者的「演示回放 · 结果已核验」标识呈现，并说明它基于一次已完成的真实分析。
